...
class Formation(object):
    ...
    def get_leader_id(self):
        return self.formation.sockets['1'].hero
    ...
    def set_socket(self, socket_id, tp, element_id):
        if tp == protomsg.SOCKET_HERO:
            self.up_hero(socket_id, element_id)
        elif tp == protomsg.SOCKET_HORSE:
            self.up_horse(socket_id, element_id)
        else:
            self.up_equipment(socket_id, element_id)
    ...
    def find_socket_by_horse(self, horse_id):
        for k, v in self.formation.sockets.iteritems():
            if v.horse == horse_id:
                return v
        return None
    ...
    def _msg_socket(self, msg, _id, socket):
        ...
        msg.horse_id = socket.horse or 0
